...
/// One wording for the zip-bomb refusal, shared by the buffered and streaming
/// paths so the same attack reads the same way whichever route it takes.
fn oversized(e: &Entry) -> String {
    format!(
...
///
/// The size cap is not decoration. The declared size is only a hint, and a
/// hostile archive simply understates it, so a few hundred bytes of DEFLATE can
/// expand to gigabytes. `set_limit` stops the decoder rather than the allocator.
fn inflate_all(raw: &[u8], e: &Entry, method: u16) -> Result<Vec<u8>, String> {
    ...
    match method {
        ...
        8 => {
            ...
            let out = DeflateDecoder::new_with_options(raw, opts)
                .decode_deflate()
                .map_err(|err| match err.error {
                    // The limit above is the entry's own declared size, so
                    // hitting it means exactly one thing. Say that, rather than
                    // passing the decoder's internal byte counts to someone who
                    // only wants to know whether to trust the file.
                    DecodeErrorStatus::OutputLimitExceeded(..) => oversized(e),
                    other => format!("{}: deflate error: {:?}", e.name, other),
                })?;
            if out.len() as u64 > e.usize_ {
                return Err(oversized(e));
            }
            Ok(out)
        }
        _ => Err(unsupported(e)),
...
fn stream_to<W: Write>(
    ...
    // The cap belongs here, in the one place every byte passes through, rather
    // than in each decoder's loop. Streaming turns an unbounded expansion into a
    // full volume instead of an out-of-memory abort, which is not an
    // improvement, so the limit matters on this path as much as on the buffered
    // one. Checking BEFORE the write is what keeps the oversized bytes off disk.
    let mut sink = |buf: &[u8], w: &mut W| -> Result<(), String> {
        // Saturating, not plain +: usize_ comes straight from the archive and
        // can be u64::MAX, where a wrapped sum would compare as *under* the
        // limit and wave the entry through.
        if total.saturating_add(buf.len() as u64) > e.usize_ {
            return Err(oversized(e));
        }
        hasher.update(buf);
